  ...
  attached() {
    Tracker.autorun(() => {
      ...
      if (Meteor.user()) {
        ...
        OneSignal.push(() => { if (!OneSignal.isPushNotificationsSupported()) { return; } OneSignal.isPushNotificationsEnabled((push) => { this.push = push; OneSignal.sendTags({ user: Meteor.user()._id }); }); OneSignal.showHttpPrompt(); OneSignal.on('subscriptionChange', (push) => { this.push = push; OneSignal.sendTags({ user: Meteor.user()._id }); }); });
      } else {
